    ...
    >
      ...
      {cartItems && cartItems.length > 0 ? (
        <div className="px-6 py-4 space-y-4 max-h-[calc(100vh-300px)] overflow-y-auto">
          {cartItems.map((item) => (
            <div key={item.id} className={item.status ? "relative" : "relative opacity-40"}>
              {!item.status && (
      ...
      <SheetFooter className="px-8 py-6 border-t flex flex-col gap-4">
        {cartItems && cartItems.length > 0 ? (
          ...
          <Button variant="outline" onClick={handleCartIsEmpty} className="w-full bg-primary rounded-md py-3 text-lg">
            Add items to cart
          </Button>
        )}
      </SheetFooter>
